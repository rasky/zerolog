...
const (
	// NC defines "not categorized" log events (category is ignored)
	NC            Category = 0
	maxcategories          = math.MaxUint8
)
...
func shouldCategory(cat Category, lvl Level) (ok bool, name string) {
	if int(cat) >= len(categories) {
		// an invalid category, the event should be logged, with an empty
		// category.
		return true, ""
	}
	catmx.Lock()
